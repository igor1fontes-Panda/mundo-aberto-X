...
- Testar sempre `npm test` após tocar no `engine.js` — os 44 testes cobrem Fibonacci, Lume, LumeBrain, críticos, Gauntlet, chat PT/EN, jogador, import/export, morte, v6 (escola/skills/fauna/chunks/conduta), v6.1 (Protocolos da Continuidade), v8 (combate ligado, itens no chão, 20 profissões, mundo com 7 territórios) e v8.1 (movimento por toque, fauna com temperamentos, NPCs de ambiente com tarefas), v9 (quest board, guerra & paz com mediação, tribunal, arcos de história, Kardashev) e v9.1 (missões físicas com marcador no terreno, Tribunal com balança animada, Praça das Missões).
